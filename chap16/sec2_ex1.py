...
def walk(f, d, num_steps):
    ...
    Moves d num_steps times; returns the distance between the final location and the location at the start of the walk.
    """
    start = f.get_loc(d)
...
def drunk_test(walk_lengths, num_trials, d_class):
    ...
    num_trials an int > 0, d_class a subclass of DrunkFor each number of steps in walk_lengths, runs sum_walks with num_trials walks and prints results
    """
...
def plot_drunk_test(walk_lengths, num_trials, d_class):
    ...
    distance_from_origins = [np.mean(distances) for distances in results]

    plt.plot(walk_lengths, distance_from_origins, label="Usual_drunk")
    plt.plot(
        walk_lengths,
        [num_steps**0.5 for num_steps in walk_lengths],
        "--b",
        label="sqrt(steps)",
    )
...
plot_drunk_test(step_list, 100, Usual_drunk)
